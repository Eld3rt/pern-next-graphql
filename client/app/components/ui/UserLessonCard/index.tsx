...
const UserLessonCard: React.FC<Props> = ({ lesson, color, courseSlug }) => {
  ...
  return (
    <li
      ...
      className="user-lesson-card rounded-2xl p-4 flex flex-col justify-between max-w-[300px] sm:w-[220px] shadow-md"
    >
      <Link
        href={`/user/courses/${courseSlug}/${lesson.slug}`}
        className="user-lesson-card__link flex sm:flex-col sm:justify-between gap-3 sm:gap-1 h-full"
      >
        <span
          style={{ color: lessonNumberColor }}
          className="user-lesson-card__text text-xs sm:text-sm sm:font-semibold self-center md:self-start"
        >
          #{lessonNumber}
        </span>
        <span
